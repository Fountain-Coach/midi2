...
- **Allow-overlap escape hatch**: Pass `allowOverlap=true` to validators when interoperating with devices that intentionally share groups. Use only when the device’s descriptor or spec addendum explicitly requires it. Swift `StreamNegotiationSession` supports this via the `allowGtbOverlap` initializer flag or `negotiate(gtbDescriptor:, allowOverlap:)`.
