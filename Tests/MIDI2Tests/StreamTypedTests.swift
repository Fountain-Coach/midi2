...
final class StreamTypedTests: XCTestCase {
    func testEndpointDiscoveryRoundTrip() throws {
        ...
        let wrong = StreamBody(opcode: .endpointInfoNotification, data1: 0x12, data2: 0x34).ump(group: group)
    ...
    func testStreamConfigurationRoundTrip() throws {
        ...
        let msg = StreamConfigurationMessage(opcode: .streamConfigurationRequest, data1: 0x26, data2: 0x00)
        let group = Uint4(0x0)!
        let pkt = msg.ump(group: group)
    ...
    func testStreamConfigurationRejectsReservedBits() {
        ...
        let pktReservedBit3 = StreamBody(opcode: .streamConfigurationRequest, data1: 0b0000_1000, data2: 0x00)
            .ump(group: group)
        ...
        let pktReservedBit4 = StreamBody(opcode: .streamConfigurationRequest, data1: 0b0001_0000, data2: 0x00)
            .ump(group: group)
        ...
        let pktReservedBit7 = StreamBody(opcode: .streamConfigurationRequest, data1: 0b1000_0000, data2: 0x00)
            .ump(group: group)
        ...
        let pktInvalidProto = StreamBody(opcode: .streamConfigurationRequest, data1: 0b0100_0000, data2: 0x00)
            .ump(group: group)
        ...
        // data2 must be zero
        let pktData2NonZero = StreamBody(opcode: .streamConfigurationRequest, data1: 0x00, data2: 0xFF)
            .ump(group: group)
