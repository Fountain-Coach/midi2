...
- **TypeScript**: `applyGtbDescriptor` (in `midi2.js/src/gtb-descriptor.ts`) applies the same map into the shared GTB context consumed by the guarded decoders and can validate descriptor groups against Function Block notifications. `loadGtbDescriptorFromJson` provides a JSON-string/parsed-object entry point.
